...
class ReadingAiNotifier extends StateNotifier<ReadingAiState> {
  ...
  /// Execute a quick action (Define, Explain, Summarize)
  Future<void> quickAction({
    ...
    // Add user message showing what action was requested
    final actionLabel = action == 'define' 
        ? 'Define' 
        : action == 'explain' 
        ? 'Explain' 
        : 'Summarize';
    ...
    try {
      // Call API for quick action
      final response = await _apiService.readingQuickAction(
        action: action,
        text: text,
        bookId: bookId,
      ...
      String responseText = '';
      if (action == 'define') {
        responseText = response['definition'] ?? '';
      ...
      state = state.copyWith(
        isLoading: false,
        error: 'Failed to process action: ${e.toString()}',
      );
    }
  }
